...
public class MobileInput : MonoBehaviour  {
    ...
    [Range(0,1)]public float movementDecay = 0.5f;
    ...
    void Update() {
        #if UNITY_EDITOR 
            if(enableDeviceScreen) DeviceHandler(movementDecay);
            else                   EditorHandler(movementDecay);
        #elif UNITY_IOS
            DeviceHandler(movementDecay);
        #elif UNITY_ANDROID
            DeviceHandler(movementDecay);
        #endif
    }

    private static void DeviceHandler(float movementDecay) {
        ...
        _rel = Vector2.Lerp(_rel, _pos, movementDecay/5);
    }
    ...
    private static void EditorHandler(float movementDecay)  {
